...
--first we create a face, then a profile from the face and then we delete the original face
	local face_handle = pytha.create_poly_face(cross_sec)
	local profile = pytha.create_profile(face_handle, height)
...
--for the excess of the handrail we simply create 2 additional segments at the beginning and at the end of the polyline. This corressponds to 1/6 of the angle of a step
	...
	local rail_edges = pytha.create_poly_edges(points,false)	--create the polyline
	if data.handrail_type < 2 then
		local sweep_info = {}
		if data.handrail_type == 0 then
			sweep_info = { type = "circle", radius = 21.2, segments = 18}	--data for the cross section of the handrail
		else 
			sweep_info = { type = "rectangle", length = 60, width = 40}	--data for the cross section of the handrail
		end
		local rail = pytha.create_sweep(rail_edges, sweep_info)		--create the sweep
