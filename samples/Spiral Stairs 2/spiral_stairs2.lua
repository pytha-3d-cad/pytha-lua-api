...
function main()
	local data = {
		...
		diameter = 3000,			--diameter of the stairs to the center of the handrail. There is an additional offset (see outer_radius)
...
function test_dialog(dialog, data)
	...
	local label5 = dialog:create_label(3, "Inner Diameter")
	local diameter2 = dialog:create_text_box(4, pyui.format_length(data.inner_diameter))
...
--here we create the points on the outside
	for i=0,segs,1 do
		x = outer_radius * COS(- angle / 2 - 30 /outer_radius * 180 / math.pi + i * outer_phi)
		y = outer_radius * SIN(- angle / 2 - 30 /outer_radius * 180 / math.pi + i * outer_phi)
		table.insert(cross_sec, {x, y,z})
	end
--here we create the points on the inside
	for i=segs,0,-1 do
		x = inner_radius * COS(- angle / 2 - 30 /inner_radius * 180 / math.pi + i * inner_phi)
		y = inner_radius * SIN(- angle / 2 - 30 /inner_radius * 180 / math.pi + i * inner_phi)
		table.insert(cross_sec, {x, y,z})
	end
...
function recreate_geometry(data)
	...
	local inner_radius = data.inner_diameter / 2	
	...
	local step_height = data.total_height / data.steps	--as close as possible to 180mm
...
--for the excess of the handrail we simply create 2 additional segments at the beginning and at the end of the polyline. This corressponds to 1/6 of the angle of a step
	for i=-2,segs+2,1 do		
		...
		z = 50 + i * data.total_height / (segs)	--this is the negative position of the outer steel construction
	...
	for i=-1,segs + 1,1 do		
		...
		z = (data.rail_height + step_height - 20 + 250) / 2 + i * data.total_height / (segs)	--this is the negative position of the outer steel construction
